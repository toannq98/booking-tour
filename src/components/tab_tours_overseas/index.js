...
import style from './tab_tours_overseas.module.scss';
import { useSelector, useDispatch } from 'react-redux';
import { useEffect, useState } from 'react';
import { categoriesSelecter, toursByCategoryOverseasSelecter } from '~/redux/tour/tour_selecter';
import PaginatedItems from '../paginated_items';

const cx = classNames.bind(style);

function TabToursOverseas({ cateParentId, subCateActiveId }) {
    const dispatch = useDispatch();
    const toursByCategory = useSelector(toursByCategoryOverseasSelecter);
    ...
    useEffect(() => {
        dispatch({
            type: "fetchTourByCategoryOverseas",
            payload: { cateId: key }
        })
    }, [dispatch, key]);
...
export default TabToursOverseas;